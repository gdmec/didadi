...
//随机抽题，题目+选项，一次抽五个,要保证题目no的连续，否则会有选项数量缺失。
...
exports.main = async (event, context) => {
  ...
  ids = ids.map(val => { return val.total })
  result=[]
  console.log('class:' + (new Date() - begin) )
  //抽题，抽答案选项
  for(let i=0;i<5;i++){
    rand = getRandomInt(1, questions.total)
    //q = await questCollection.skip(rand).limit(1).get()
    //console.log('skip:' + (new Date() - begin))
    //randno=q.data[0].no
    //id = Math.floor(randno/10000)
